...
import com.HomeHarbour.HomeHarbourApp.entity.*;
...
import com.HomeHarbour.HomeHarbourApp.repository.*;
import com.HomeHarbour.HomeHarbourApp.strategy.PricingService;
...
public class BookingServiceImplementation implements BookingService{
    ...
    public BookingStatus getBookingStatus(Long bookingId) {
        Booking booking = bookingRepository.findById(bookingId).orElseThrow(
                () -> new ResourceNotFoundException(STR."Booking not found with id: \{bookingId}")
        );
        User user = getCurrentUser();
        if (!user.equals(booking.getUser())) {
            throw new UnAuthorisedException(STR."Booking does not belong to this user with id: \{user.getId()}");
        }
    ...
    public List<BookingDto> getAllBookingsByHotelId(Long hotelId) {
        Hotel hotel = hotelRepository.findById(hotelId).orElseThrow(() -> new ResourceNotFoundException(STR."Hotel not found with ID: \{hotelId}"));
        User user = getCurrentUser();

        log.info("Getting all booking for the hotel with ID: {}", hotelId);

        if(!user.equals(hotel.getOwner())) throw new AccessDeniedException(STR."You are not the owner of hotel with id: \{hotelId}");
